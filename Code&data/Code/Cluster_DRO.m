...
multiplier = 0.08;  %The factor to control the parameter \theta
    ...
    if k==4
        for n = 1:size(DiffertKK{k,1},1)
            ...
            for j = 1:size(DiffertKK{k,2},1) % Constraints  when k==1
                ...
                F = [F, alpha4(n) >= temp - lamda * norm( DiffertKK{k,2}(j,:) -  DiffertKK{k,1}(n,:) ,1)  ]  ;   % every j  , k,  n has a corresponding constraint
                F = [F, sum(yij4(:,j)) == 1] ;
            end
        end
    end
    ...
    if k==5
        for n = 1:size(DiffertKK{k,1},1)
            ...
            for j = 1:size(DiffertKK{k,2},1) % Constraints  when k==1
                ...
                F = [F, alpha5(n) >= temp - lamda * norm( DiffertKK{k,2}(j,:) -  DiffertKK{k,1}(n,:) ,1)  ]  ;   % every j  , k,  n has a corresponding constraint
                F = [F, sum(yij5(:,j)) == 1] ;
            end
        end
    end
