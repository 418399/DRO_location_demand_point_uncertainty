...
while stop==0
    ...
    [xi,yij, yn, k, Obj] = optimize_RSM(EmProba, DC, Sbar, His, b, tau)  ;
    savesolutions{iter,1} = xi ; %记录每次迭代的解和目标函数值
    savesolutions{iter,2} = yn ;
    ...
    for n =1:size(His,1)
        ...
        for j = 1:size(LC,1)  %计算 k>=() 中 ()中的项，找到()在已知 yn xi 下的最大值
            ...
            temp(j) = (min_value(j) - yn(n) ) / save_1_norm(j) ;
        end
    ...
    if sum(if_feasibleNum) < size(His,1)
        ...
        for j = 1:size(Zn,1)  %验证每个 z_j 所产生的 sn ，结合新的Kn, 以及松弛解（x_i）所组成的解是否满足约束。
            for n = 1:size(His,1)
