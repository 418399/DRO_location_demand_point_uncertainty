function Vcells = Voronoi(nodes,area )
%
...
nodesExtended(N+1,2)=-10000000;...needed for Voronoi computation of Infinite Cells
    nodesExtended(N+2,2)=-10000000;...%...
    nodesExtended(N+3,2)=10000000;...%...
    nodesExtended(N+4,2)=10000000;...
    nodesExtended(N+1,1)=-10000000;...
    nodesExtended(N+2,1)=10000000;...
    nodesExtended(N+3,1)=-10000000;...
    nodesExtended(N+4,1)=10000000;...
...
%         if inAreaFlag(i) == 1
%             plot(vertices(i,1),vertices(i,2),'y+','LineWidth',1)
%         end
%     end
...
%         plot(area(k:k+1,1),area(k:k+1,2),'k-','LineWidth',1)
%     end
   % plot([area(end,1) area(1,1)], [area(end,2) area(1,2)],'k-','LineWidth',1)
  %  plot(nodes(1:N,1),nodes(1:N,2),'bv','MarkerSize',5)
  % for i = 1:N; text(nodes(i,1),nodes(i,2),['C' num2str(i) ' '],'HorizontalAlignment','right');end % Ploting number of node (same as cell)
    ...
    for e=1:1:N
     ...
     plot(cell2mat(Vcells(e,1)),cell2mat(Vcells(e,2)),'k-') ;
        
    end
  %  axis equal
%  xlim([min(area(:,1),[],1), max(area(:,1),[],1)  ])
% ylim([min(area(:,2),[],1), max(area(:,2),[],1)])
hold off;
